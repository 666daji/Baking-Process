...
public class PlatingProcess<T extends BlockEntity & PlatableBlockEntity> extends AbstractProcess<T> {
    ...
    /** 当前步骤的候选配方列表 */
    private final List<RecipeHolder<PlatingRecipe>> candidateRecipes = new ArrayList<>();

    /** 当前完全匹配的配方（如果存在） */
    @Nullable
    private RecipeHolder<PlatingRecipe> matchedRecipe = null;
    ...
    protected class PerformActionStep implements Step<T> {
        ...
        public StepResult execute(StepExecutionContext<T> context) {
            ...
            // 候选列表已初始化
                ...
                // 使用预期操作过滤候选列表
                List<RecipeHolder<PlatingRecipe>> matchingRecipes = filterCandidatesByNextAction(
    ...
    public boolean initializeCandidates(Level world, PlatableBlockEntity plate, @Nullable PlayerAction expectedAction) {
        ...
        try {
            ...
            List<RecipeHolder<PlatingRecipe>> allRecipes = recipeManager.getAllRecipesFor(ModRecipeTypes.PLATING.get());
            ...
            List<RecipeHolder<PlatingRecipe>> candidates = allRecipes.stream()
                    .filter(recipe -> recipe.value().getContainer() == containerType)
                    .filter(recipe -> recipe.value().matchesPrefix(tempMatchingList))
                    .toList();
     ...
     */
    private List<RecipeHolder<PlatingRecipe>> filterCandidatesByNextAction(PlayerAction nextAction, List<PlayerAction> performedActions) {
        return candidateRecipes.stream()
                .filter(recipe -> {
                    // 如果已执行操作数量 >= 配方操作数量，不是有效候选
                    if (performedActions.size() >= recipe.value().getActionCount()) {
                        return false;
                    }

                    // 检查已执行操作是否是配方的有效前缀
                    if (!recipe.value().matchesPrefix(performedActions)) {
                        return false;
                    }

                    // 检查下一步是否匹配
                    PlayerAction nextRecipeAction = recipe.value().getNextAction(performedActions.size());
    ...
    public void checkForExactMatch(PlatableBlockEntity plate, Level world) {
        matchedRecipe = candidateRecipes.stream()
                .filter(recipe -> recipe.value().matches(plate, world))
                .findFirst()
                .orElse(null);
    }
     ...
     * 获取当前匹配的配方。
     */
    public @Nullable RecipeHolder<PlatingRecipe> getMatchedRecipe() {
    ...
    protected String getCustomStatusInfo() {
        ...
        // 匹配的配方信息
        if (matchedRecipe != null) {
            info.append("完全匹配的配方: ").append(matchedRecipe.id().getPath()).append("\n");
            info.append("配方操作数: ").append(matchedRecipe.value().getActionCount()).append("\n");
            info.append("输出菜肴: ").append(matchedRecipe.value().getDishes()).append("\n");
        } else {
        ...
        if (!candidateRecipes.isEmpty()) {
            ...
            for (int i = 0; i < limit; i++) {
                RecipeHolder<PlatingRecipe> recipe = candidateRecipes.get(i);
                info.append("  ").append(i + 1).append(". ")
                        .append(recipe.id().getPath())
                        .append(" (操作: ").append(recipe.value().getActionCount()).append(")\n");
            }
