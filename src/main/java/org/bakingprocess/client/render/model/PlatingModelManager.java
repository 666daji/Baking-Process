...
public class PlatingModelManager {
     ...
     */
    private final Map<SequenceKey, ResourceLocation> sequenceCache = new HashMap<>();
     ...
     */
    private final Table<Item, String, ResourceLocation> recipeModelCache = HashBasedTable.create();
     ...
     */
    @Nullable
    public ResourceLocation getModelForActions(Item container, List<PlayerAction> actions) {
        // 参数验证
        ...
        ResourceLocation recipeModel = tryMatchRecipeModel(container, codeSequence);
        if (recipeModel != null) {
        ...
        // 检查操作序列缓存
        ResourceLocation cachedModel = sequenceCache.get(key);
        if (cachedModel != null) {
            return cachedModel;
        }

        // 生成步骤模型路径
        ResourceLocation modelPath = generateModelPath(container, codeSequence);
    ...
    public void registerRecipeModel(Item container, List<PlayerAction> recipeActions, Content dish) {
        ...
        ResourceLocation dishId = ModModelLoader.createDishesModel(container, dish);
     ...
     */
    public List<ResourceLocation> generateAllPrefixModels(Item container, List<PlayerAction> actionSequence) {
        ...
        for (int i = 1; i <= actionSequence.size(); i++) {
            ...
            ResourceLocation modelId = getModelForActions(container, prefix);
            if (modelId != null) {
                models.add(modelId);
            }
        }
     ...
     */
    @Nullable
    private ResourceLocation tryMatchRecipeModel(Item container, List<String> codeSequence) {
        // 生成当前序列的哈希
     ...
     * 生成模型路径标识符
     */
    private ResourceLocation generateModelPath(Item container, List<String> codeSequence) {
        // 获取容器编码
    ...
    private String getContainerCode(Item container) {
        ResourceLocation containerId = BuiltInRegistries.ITEM.getKey(container);
    ...
    private static class SequenceKey {
        ...
        public String toString() {
            return String.format("SequenceKey{container=%s, actions=%d}",
                    BuiltInRegistries.ITEM.getKey(container), actions.size());
        }
    }
}