...
public class FlourSackTooltipComponent implements ClientTooltipComponent {
    ...
    public void renderImage(Font textRenderer, int x, int y, GuiGraphics context) {
        ...
        if (this.content.isPresent()) {
            ...
            context.renderItem(itemStack, itemX, itemY, 0);
            context.renderItemDecorations(textRenderer, itemStack, itemX, itemY);
        }
    }

    private void drawSlot(int x, int y, boolean isBlocked, GuiGraphics context) {
        // 绘制槽位边框
        this.drawBorder(x, y, context);

        // 绘制槽位内部
        Sprite sprite = isBlocked ? Sprite.BLOCKED_SLOT : Sprite.SLOT;
        context.blit(TEXTURE, x + BORDER_SIZE, y + BORDER_SIZE, 0,
                sprite.u, sprite.v, sprite.width, sprite.height, 128, 128);
    }

    private void drawBorder(int x, int y, GuiGraphics context) {
        // 绘制四个角的边框
    ...
    private void draw(GuiGraphics context, int x, int y, Sprite sprite) {
        context.blit(TEXTURE, x, y, 0, sprite.u, sprite.v, sprite.width, sprite.height, 128, 128);
    }
