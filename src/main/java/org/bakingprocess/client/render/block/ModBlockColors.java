...
public class ModBlockColors {
    public static void registryColors(RegisterColorHandlersEvent.Block event) {
        event.register(ModBlockColors::getFlourSackColor, ModBlocks.FLOUR_SACK.get());
        event.register((state, world, pos, tintIndex) -> tintIndex != -1 ? 4159204 : -1, AssistedBlocks.CRIPPLED_WATER_BUCKET.get());
        event.register(ModBlockColors::getPotColor, ModBlocks.IRON_POTS.get(), ModBlocks.CLAY_POTS.get());
    }

    private static int getFlourSackColor(BlockState state, @Nullable BlockAndTintGetter world, @Nullable BlockPos pos, int tintIndex) {
        // 检查必要的参数
    ...
    private static int getShelfFlourSackColor(UpPlaceBlockEntity shelfBlockEntity, BlockState state) {
        // 获取架子索引
        int shelfIndex = 0;
        if (state.hasProperty(FlourSackBlock.SHELF_INDEX)) {
            shelfIndex = state.getValue(FlourSackBlock.SHELF_INDEX);
        }

        // 检查该槽位是否有粉尘袋
        if (shelfIndex < 0 || shelfIndex >= shelfBlockEntity.getContainerSize()) {
            return FlourSackBlockEntity.DEFAULT_FLOUR_COLOR;
        }

        ItemStack shelfStack = shelfBlockEntity.getItem(shelfIndex);
    ...
    private static int getPotColor(BlockState state, @Nullable BlockAndTintGetter world, @Nullable BlockPos pos, int tintIndex) {
