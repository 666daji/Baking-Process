...
public abstract class AbstractMappedContainer extends ContainerType {
    ...
    public void registerContentMapping(AbstractContent content, Item item) {
        if (!canContain(content)) {
            BakingProcess.LOGGER.warn("Attempted to register invalid content to {} container: {}",
                    getId(), content.getId());
            return;
        }
        ...
        if (item == getEmptyItem()) {
            BakingProcess.LOGGER.error("Cannot register empty container item as content mapping: {}", item);
            return;
        }
    ...
    public @NotNull ItemStack replaceContent(@NotNull ItemStack stack, @Nullable AbstractContent content) {
        ...
        if (item == null) {
            // 内容物没有对应的物品映射，无法替换
            BakingProcess.LOGGER.warn("No item mapping found for content: {} in container {}",
                    content.getId(), getId());
            return stack.copy();
        }
